...
import { IPriceSelectionStrategy, TransactionBaseService } from "@medusajs/medusa/dist/interfaces"
...
import NewStoreService from "./new-store"
type CustomType = { variantId: string, quantity: number }
type CurrencyRate = {
  code: string
  name: string
  rate: string
...
class CartService extends TransactionBaseService {
  ...
  protected readonly newStoreService_: NewStoreService
  ...
  constructor({
    ...
    this.newStoreService_ = newStoreService;
  }
    ...
    return await this.atomicPhase_(
      async (transactionManager: EntityManager) => {
        ...
        if (!rawCart.email && data.email) {
          const customer = await this.createOrFetchGuestOrExistingCustomerFromEmail_(
            data.email
          )
          rawCart.customer = customer
          rawCart.customer_id = customer.id
   ...
   */
  protected async createOrFetchGuestOrExistingCustomerFromEmail_(
    email: string
  ): Promise<Customer> {
    ...
    let registered_customer = await customerServiceTx
      .retrieveRegisteredByEmail(validatedEmail)
      .catch(() => undefined)
    if (registered_customer) return registered_customer;
  ...
  protected async updateUnitPrices_(
    ...
    const getCurrentRate = async (currencyCode: string): Promise<CurrencyRate | null> => {
      var store = await this.newStoreService_.list();
      const { storeCurrencies } = store;
      const { currencies } = storeCurrencies[0]
      var currency: any = currencies.find((c) => c.code === currencyCode.toLowerCase());
      return currency;
    }
    ...
    // value if it exists)
    var currentRegion = cart.region;
    var previousRate = await getCurrentRate(currentRegion.currency_code)
    ...
    var nextRate = await getCurrentRate(region.currency_code)
    const lineItemServiceTx = this.lineItemService_.withTransaction(
      this.activeManager_
    )

    // const calculateVariantPriceData: CustomType[] = cart.items
    //   .filter((i) => i.variant_id)
    //   .map((item) => {
    //     return { variantId: item.variant_id!, quantity: item.quantity }
    //   })

    const availablePriceMap = async (item: LineItem): Promise<any> => {
      var product_id = item.variant.product_id;
      var product = await this.productService_.retrieve(product_id);
      var collection = await this.productCollectionService_.retrieve(product.collection_id)
      if (collection.handle === 'custom-fightwear' || collection.handle === 'custom-equipment') {
        const actualPrice = Math.round((item.unit_price / Number(previousRate.rate)) * Number(nextRate.rate));
        return {
      ...
      return await this.priceSelectionStrategy_
        .withTransaction(this.activeManager_)
    ...
    cart.items = (
      await Promise.all(
        cart.items.map(async (item) => {
          ...
          const availablePrice = await availablePriceMap(item);
          if (
