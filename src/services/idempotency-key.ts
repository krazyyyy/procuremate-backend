...
} from "@medusajs/medusa/dist/types/idempotency-key"
import { DeepPartial, EntityManager } from "typeorm"
import { MedusaError, isDefined } from "medusa-core-utils"
import { buildQuery, isString } from "@medusajs/medusa/dist/utils"

import { IdempotencyKey } from "@medusajs/medusa/dist/models"
...
class IdempotencyKeyService extends TransactionBaseService {
  ...
  async clearFinished(): Promise<number> {
    ...
    var finsihedKeys = await this.idempotencyKeyRepository_.find({ where: { recovery_point: 'finished' } })
    for (var key of finsihedKeys) {
      var res = await this.idempotencyKeyRepository_.delete(key.id);
      if (res.affected === 1) {
        count += res.affected;
      }
    }
