...
import { CreatePaymentInput, PaymentSessionInput } from "@medusajs/medusa/dist/types/payment"
  ...
  >(
    ...
    return await this.atomicPhase_(async (transactionManager) => {
      ...
      if (provider instanceof AbstractPaymentProcessor) {
        paymentResponse = await provider.initiatePayment(params)


        if ("error" in paymentResponse) {
          delete context.customer?.metadata?.stripe_id
          paymentResponse = await provider.initiatePayment(params);
          if ('error' in paymentResponse) {
