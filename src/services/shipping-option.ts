...
import { TransactionBaseService } from "@medusajs/medusa/dist/"
...
} from "@medusajs/medusa/dist/types/shipping-options"
...
class ShippingOptionService extends TransactionBaseService {
  ...
  async validateCartOption(
    ...
    const express = !option.name.toLowerCase().includes('free') || option.name.toLowerCase().includes('express')
    if (cart.items.length > 1 && express || (cart.items.find((c) => c.quantity > 1 && express))) {
      option.amount = (option.amount * 2) * 0.8
    }
    return option
  }
